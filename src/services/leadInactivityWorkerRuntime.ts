...
import { isInactivityDailyCapDisabled, isInactivityMovementPaused } from "./inactivityMovementSwitch";
import {
...
function createProductionWorker(
  ...
  // The one notification every administrator receives: a lead moving into the
  // Phoenix pipeline. Everything else is operator-only.
  return createLeadInactivityWorker({
    store,
    amo,
    isMovementPaused: () => isInactivityMovementPaused(),
    isDailyCapDisabled: () => isInactivityDailyCapDisabled(),

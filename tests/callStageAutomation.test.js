...
function baseDependencies(overrides = {}) {
  ...
  return {
    ...
    store: {
      ...
      async reserveTestMove() { return { kind: "reserved", slot: { slotNumber: 1 } }; },
...
test("a clear target with missing amoCRM-required fields alerts admins and consumes no test move slot", async () => {
  ...
  const dependencies = baseDependencies({
    store: {
      async getActivationBoundary() { return boundary; },
      async reserveTestMove() { reserved = true; return { kind: "reserved", slot: { slotNumber: 1 } }; },
...
test("a clear eligible UZUM outcome reserves one test slot, moves once, verifies, notes, and confirms capacity", async () => {
  ...
  const dependencies = baseDependencies({
    store: {
      async getActivationBoundary() { return boundary; },
      async reserveTestMove() { calls.reserve += 1; return { kind: "reserved", slot: { slotNumber: 1 } }; },
...
test("an ambiguous latest call is a durable no-op and never alerts or claims movement capacity", async () => {
  ...
  const dependencies = baseDependencies({
    ...
    store: {
      async getActivationBoundary() { return boundary; },
      async reserveTestMove() { reservationCalled = true; throw new Error("must not reserve"); },
